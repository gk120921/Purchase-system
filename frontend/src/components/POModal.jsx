...
export default function POModal({ user, editData, isPreview, onClose, suppliers = [], materials = [], onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    po_number: editData ? (editData.po_number || editData.number || '') : `PO-${Date.now()}`,
        ...
        const populateData = (items) => {
          setFormData(prev => {
            const next = {
              ...
              remarks: fullData.remarks || prev.remarks || '',
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.5rem', marginBottom: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '8px' }}>
            ...
            <div style={{ gridColumn: 'span 3' }}>
              ...
              />
              <div className="print-only" style={{ padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '4px', minHeight: '60px', whiteSpace: 'pre-wrap' }}>
            ...
            <tbody>
              {formData.items.map((item, index) => (
                <tr key={index}>
                  ...
                  <td style={{ border: '1px solid #000', padding: '0.5rem', textAlign: 'left' }}>
